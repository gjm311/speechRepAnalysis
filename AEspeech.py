...
#             print("Scalers not found..")
    ...
    def compute_spectrograms(self, wav_file, plosives_only=1, volta=0,rsmp=0):
        """
        ...
        if rsmp==1:
            signal=librosa.core.resample(signal.astype(float),44100,FS)
        sig_len=len(signal)
        if fs_in!=FS:
#             signal=librosa.core.resample(signal.astype(float),44100,FS)
    ...
    def show_spectrograms(self, sig_len, spectrograms1, spectrograms2=None, title="",volta=0):
        """
...
#                     ax.set_title("Reconstructed Continuous Wavelet Transform",fontsize=16)
    ...
    def compute_bottleneck_features(self, wav_file, return_numpy=True,rsmp=0):
        """
        ...
        if self.rep=='mc_fuse':
            self.rep='broadband'
            bb_mat=self.compute_spectrograms(wav_file,rsmp=rsmp)
            bb_mat=self.standard(bb_mat)
            self.rep='narrowband'
            nb_mat=self.compute_spectrograms(wav_file,rsmp=rsmp)
    ...
    def compute_rec_error_features(self, wav_file,plosives_only=1, return_numpy=True,rsmp=0):
        """
        ...
        if self.rep=='mc_fuse':
            self.rep='broadband'
            bb_mat=self.compute_spectrograms(wav_file,plosives_only=plosives_only,rsmp=rsmp)
            bb_mat=self.standard(bb_mat)
            self.rep='narrowband'
            nb_mat=self.compute_spectrograms(wav_file,plosives_only=plosives_only,rsmp=rsmp)
            nb_mat=self.standard(nb_mat)
            self.rep='mc_fuse'
...
#             error=[np.correlate(mat_mean[i,:].detach().numpy(), to_mean[i,:].detach().numpy()) for i in range(mat.shape[2])]
    ...
    def compute_dynamic_features(self, wav_directory,rsmp=0):
        """
        ...
        for cnt, wav_file in enumerate(hf):
            if cnt>=7 and 'HC' in wav_file and rsmp==1:
                bottle=self.compute_bottleneck_features(wav_directory+wav_file, True,rsmp=rsmp)
                error=self.compute_rec_error_features(wav_directory+wav_file, True, rsmp=rsmp)
            else:
